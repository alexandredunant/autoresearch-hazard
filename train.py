...
def fit_ebm(X: np.ndarray, y: np.ndarray, feature_names: list[str], model_config: dict[str, Any], seed: int):
...
def random_split_eval(X, y, feature_names, model_config, seed: int) -> tuple[dict[str, float], Any]:
    train_idx, val_idx = train_test_split(
        np.arange(len(y)), test_size=0.2, stratify=y, random_state=seed
    )
    model = fit_ebm(X[train_idx], y[train_idx], feature_names, model_config, seed)
...
def temporal_audit_eval(X, y, metadata, feature_names, model_config, seed: int) -> dict[str, float]:
...
def spatial_audit_eval(X, y, metadata, feature_names, model_config, seed: int) -> dict[str, float]:
...
def train_one(config, process, experiment, args) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    random_metrics, split_model = random_split_eval(
        X, y, selected, experiment.MODEL_CONFIG, config.random_state
    )
    temporal_metrics = temporal_audit_eval(
        X, y, metadata, selected, experiment.MODEL_CONFIG, config.random_state
    ) if args.with_audit else {}
    spatial_metrics = spatial_audit_eval(
        X, y, metadata, selected, experiment.MODEL_CONFIG, config.random_state
    ) if args.with_audit else {}
    final_model = fit_ebm(X, y, selected, experiment.MODEL_CONFIG, config.random_state)
    result = {
        "process": process.key,
